...
| Project ID | Role |
|---|---|
| `gcp-project-runtime` (display "Unity LiveKit") | Main runtime: GKE cluster `unity`, Cloud Run `droid-comms-app`/`droid-adapters` (+`-staging`), Pub/Sub fleet, most buckets, tunnel servers, Artifact Registry |
| `gcp-project-vms` | Assistant desktop VM pool: pool images/families, pool VMs, static IPs, per-assistant archives |
| `gcp-project-saas` ("SaaS") | **Orchestra + Console + landing page** (Cloud Run) and **Cloud SQL** Postgres (`prod-ssd-usc1`/`staging-ssd-usc1`, us-central1) |
| `gcp-project-dns` | Public DNS zone `unifyai` → `unify.ai` (incl. `vm.unify.ai`, `tunnel.unify.ai`) |
...
- **Cloud Build is the #1 trap — it's regional and the `global` default lies.** All triggers + builds for **both** `gcp-project-runtime` and `gcp-project-saas` are in **`us-central1`**. With no `--region`, `gcloud builds …` hits `global`, where `gcp-project-runtime` is **empty/months-stale** and `gcp-project-saas` shows **only `landing-page`** (hiding all `orchestra`/`console` builds). Always: `gcloud builds {list,describe,log,triggers list,triggers run} --region=us-central1`.
...
- **Assistant desktops**: pooled Ubuntu/Windows VMs in `gcp-project-vms`; runtime syncs `~/Unity/Local` over rclone SFTP (user `unityuser`, port 2222); cross-session home persisted to `gs://bucket/{id}.tar.gz`; optional rathole tunnel relay (`unity-tunnel-server`) for user machines.
- **Backend/UI/DB** all in `gcp-project-saas`. Orchestra at `https://api.unify.ai/v0`.
...
project IDs `gcp-project-vms` & `gcp-project-runtime`, all service-account emails
(`pool-vm-sa@gcp-project-vms`, `comm-sa@gcp-project-runtime`), and GKE cluster `unity`
(`DROID_GKE_CLUSTER_NAME` default `unity`).

**Canonical names that commonly confuse (use these, verify before assuming):**
- GKE cluster: `unity`; VM/image project: `gcp-project-vms`.
...
Orchestra (`api.unify.ai/v0` prod, `internal.example.com/v0` staging) has **two distinct auth paths** (`orchestra/web/api/dependencies.py`). Confusing them wastes hours.
...
- The live `ORCHESTRA_ADMIN_KEY` is the **GCP secret** in `gcp-project-saas` (prod) — repo `.env` copies may be stale. Staging uses a different value. Fetch with `gcloud secrets versions access latest --secret=ORCHESTRA_ADMIN_KEY --project=gcp-project-saas`.
...
A cross-tenant migration = loop assistants from `/admin/assistant`, then operate per-assistant with each key (no superuser data key exists). `gcloud` Cloud SQL (`prod-ssd`, `gcp-project-saas`) is the direct-DB fallback for bulk passes.
