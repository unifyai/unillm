...
from unillm.clients.uni_llm import (
    _DEEPSEEK_R1_0528_HARD_OPENROUTER_PROVIDERS,
...
DEEPSEEK_R1_ENDPOINT = "deepseek-r1@vertex-ai"
DEEPSEEK_R1_PROVIDER_MODEL = "deepseek/deepseek-r1-0528"
...
def test_deepseek_r1_0528_alias_registered() -> None:
    assert get_model_alias(DEEPSEEK_R1_ENDPOINT) == (
        f"openrouter/{DEEPSEEK_R1_PROVIDER_MODEL}"
    )
    assert "deepseek-r1" in list_models("vertex-ai")


def test_deepseek_r1_0528_openrouter_pins_hard_providers() -> None:
    transport_model = get_transport_model_alias(DEEPSEEK_R1_ENDPOINT)
    kw = {
        "model": transport_model,
        "messages": [{"role": "user", "content": "hello"}],
    }

    _prepare_provider_request_kw(kw=kw, provider="vertex-ai", stream=False)

    assert transport_model.startswith("openrouter/")
    assert "deepseek-r1-0528" in transport_model
    assert kw["extra_body"]["provider"] == {
        "order": list(_DEEPSEEK_R1_0528_HARD_OPENROUTER_PROVIDERS),
        "allow_fallbacks": False,
    }


def test_sync_deepseek_r1_0528_simple_message() -> None:
    client = unillm.Unify(DEEPSEEK_R1_ENDPOINT, temperature=0)
...
@pytest.mark.asyncio
async def test_async_deepseek_r1_0528_simple_message() -> None:
    client = unillm.AsyncUnify(DEEPSEEK_R1_ENDPOINT, temperature=0)
...
def test_deepseek_r1_0528_enforces_tool_choice_required_adversarially() -> None:
    ...
    assert message.tool_calls, (
        "Pinned DeepSeek R1-0528 host did not hard-enforce "
...
def test_deepseek_r1_0528_enforces_response_format_adversarially() -> None:
