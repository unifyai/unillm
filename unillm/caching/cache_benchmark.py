import functools
import os
import warnings
from dataclasses import dataclass


@dataclass
class CacheStats:
    hits: int = 0
    # Hits accepted through canonical keying rather than an exact raw-key
    # match; a subset of `hits`. A rising share is the prompt-drift meter.
    canonical_hits: int = 0
    misses: int = 0
    reads: int = 0
    writes: int = 0

    def get_percentage_of_cache_hits(self) -> float:
        if self.reads == 0:
            return 0.0
        return self.hits / self.reads * 100

    def get_percentage_of_cache_misses(self) -> float:
        if self.reads == 0:
            return 0.0
        return self.misses / self.reads * 100

    def __repr__(self) -> str:
        return f"CacheStats(hits={self.hits} ({self.get_percentage_of_cache_hits():.1f}%, {self.canonical_hits} canonical), misses={self.misses} ({self.get_percentage_of_cache_misses():.1f}%), reads={self.reads}, writes={self.writes})"

    def __add__(self, other: "CacheStats") -> "CacheStats":
        return CacheStats(
            hits=self.hits + other.hits,
            canonical_hits=self.canonical_hits + other.canonical_hits,
            misses=self.misses + other.misses,
            reads=self.reads + other.reads,
            writes=self.writes + other.writes,
        )


CURRENT_CACHE_STATS = CacheStats()


def _is_cache_stats_enabled() -> bool:
    return os.environ.get("UNILLM_CACHE_STATS", "true").lower() in ("true", "1")


def get_cache_stats() -> CacheStats:
    if not _is_cache_stats_enabled():
        warnings.warn(
            "Cache stats is not enabled, set UNILLM_CACHE_STATS=true to enable it, must be set before importing unillm.",
        )
    return CURRENT_CACHE_STATS


def record_get_cache(fn):
    if not _is_cache_stats_enabled():
        return fn
    else:

        @functools.wraps(fn)
        def wrapper(*args, **kwargs):
            benchmark = get_cache_stats()
            benchmark.reads += 1
            value, hit_kind = fn(*args, **kwargs)
            if value is None:
                benchmark.misses += 1
            else:
                benchmark.hits += 1
                if hit_kind == "canonical":
                    benchmark.canonical_hits += 1
            return value, hit_kind

        return wrapper


def record_write_to_cache(fn):
    if not _is_cache_stats_enabled():
        return fn
    else:

        @functools.wraps(fn)
        def wrapper(*args, **kwargs):
            benchmark = get_cache_stats()
            benchmark.writes += 1
            return fn(*args, **kwargs)

        return wrapper
