#!/usr/bin/env python3
"""
Consolidate LLM cache files from CI artifacts.

Merges new cache entries (.cache_write.ndjson) from test runs into the
existing global cache (.cache.ndjson). Duplicate entries are deduplicated
...
def find_cache_files(artifacts_dir: Path) -> List[Path]:
    """
    Find all diff cache files (".cache_write.ndjson") under the artifacts directory.
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Merge existing global cache with diff ndjson files from artifacts directory",
    )
    parser.add_argument(
        "--artifacts-dir",
        required=True,
